...
export class TopicContentComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, private topicsApi: TopicsService, private consumerApi: ConsumerService, private modalService: ModelsService, private consumerGroupService: ConsumerGroupService) { }
  ...
  private keepConsumerAlive() {
    ...
    Observable.interval(1 * 60 * 1000).subscribe(x => {
      this.consumerApi.keepAlive(this.topicName, this.consumerGroupName).subscribe(result => {
  ...
  private getTopicOffsets() {
    ...
    this.topicsApi.getOffsets("Beginning", this.topic).subscribe(result => {
    ...
    this.topicsApi.getOffsets("Ending", this.topic).subscribe(result => {
  ...
  private initalizeCosumerGroup() {
    ...
    this.consumerApi.initializeConsumerInstance(this.topicName, this.consumerGroupName, this.partitionEmptyOffsetArray).subscribe(result => {
  ...
  public fetch() {
    ...
    this.consumerApi.consumeFromTopic(this.topicName, this.consumerGroupName, this.position, this.fetchFromOffsets, this.noOfRecordsPerPartition, this.searchKey).subscribe(result => {
